...
class CubeSimulator(object):
    def __init__(self,
                 dt=1.e-3,
                 int_steps=1):
        self.blmc_robots_path = os.path.join(
            rospkg.RosPack().get_path("blmc_robots"))
    ...
    def reset_state(self, x):
        assert x.shape == (self.n, 1), BaseException(
    ...
    def increment(self, x, dx):
        ...
        assert x.shape == (self.n, 1), BaseException(
            'state vector has wrong dimension')
        assert dx.shape == (self.ndx, 1), BaseException(
            'tangent vector  has wrong dimension')
    ...
    def diff(self, x1, x2):
        """ returns x2-x1 """
        assert x1.shape == (self.n, 1), BaseException(
            'state vector 1 has wrong dimension')
        assert x2.shape == (self.n, 1), BaseException(
            'state vector 2 has wrong dimension')
    ...
    def step(self, u):
        ...
        assert u.shape == (self.m, 1), BaseException(
            'control vector has wrong dimension')
        dx = np.zeros([self.ndx, 1])
        x = self.state.copy()
        for _ in range(self.int_steps):
            pin.aba(self.rmodel, self.rdata, x[:self.nq], x[self.nq:], u)
            dx[:self.nv] = self.sub_dt * \
                (x[self.nq:] + .5 * self.sub_dt * self.rdata.ddq)
...
if __name__ == '__main__':
    ...
    print('error in final vertical velocity ',
          xt[-1, sim.nq + 2, 0] - (-2. * 9.81))
    ...
    # plot the height as function of time (should look like -x^2 for a free
    # fall )
