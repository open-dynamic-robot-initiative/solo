...
class Robot_Control:
    ...
    def initialize_robot(self):
        ...
        if self.mode == "real":
            ...
            self.finger = sim_finger.SimFinger(
        ...
        self.finger_tip_links = [
            ...
            "finger_tip_link_120",
            "finger_tip_link_240"]
        self.finger_tip_ids = [self.robot_model.getFrameId(
            name) for name in self.finger_tip_links]
    ...
    def box_state_from_tip_position(self):
        ...
        end_effector_position = [
            self.robot_data.oMf[tip_id].translation for tip_id in self.finger_tip_ids]
        block_com_position = [
            (end_effector_position[0][i] +
             end_effector_position[1][i] +
             end_effector_position[2][i]) /
            3.0 for i in range(3)]

        block_com_position = np.resize(block_com_position, (3, 1))

        unit_vector_along_x = np.resize(
            (end_effector_position[1] - end_effector_position[2]), 3)
        unit_vector_along_x = unit_vector_along_x / \
            max(np.linalg.norm(unit_vector_along_x), 1.e-9)
        ...
        unit_vector_along_z = unit_vector_along_z / \
            max(np.linalg.norm(unit_vector_along_z), 1.e-9)

        unit_vector_along_y = np.cross(
            unit_vector_along_z, unit_vector_along_x)
        unit_vector_along_y = unit_vector_along_y / \
            max(np.linalg.norm(unit_vector_along_y), 1.e-9)

        self.rotation_matrix = np.matrix(
            [unit_vector_along_x, unit_vector_along_y, unit_vector_along_z]).T
        ...
        block_com_position = block_com_position + \
            self.rotation_matrix.dot(offset)
    ...
    def finger_tip_observed_state(self):
        ...
        if self.mode == "real":
            time_stamp = self.finger.frontend.append_desired_action(
                self.finger.Action(torque=self.torque))
        ...
        J = [
            pinocchio.getFrameJacobian(
                ...
                :3] for tip_id in self.finger_tip_ids]
        self.J = np.vstack(J)
        ...
        self.x_obs = [self.robot_data.oMf[tip_id].translation -
                      self.block_com_position for tip_id in self.finger_tip_ids]
    ...
    def move(
        ...
        while True:
            # Slowly changing (indirectly) the xyz-position for the
            # end_effector by changing the position of the block
            x = (1 - math.cos(math.radians(i / 10))) * \
                (desired_position[0] - obj_start_pos[0]) / 2 + obj_start_pos[0]
            y = (1 - math.cos(math.radians(i / 10))) * \
                (desired_position[1] - obj_start_pos[1]) / 2 + obj_start_pos[1]
            z = (1 - math.cos(math.radians(i / 10))) * \
                (desired_position[2] - obj_start_pos[2]) / 2 + obj_start_pos[2]

            self.block_com_position = [x, y, z]
            self.quat = [0, 0, 0, 1]
            self.cube_q_next = np.concatenate(
                [self.block_com_position, self.quat, [0] * 6])
            self.rotation_matrix = pinocchio.XYZQUATToSe3(
                self.cube_q_next[:7]).rotation

            # Calculating desired_external_force to apply at the contact points
            # (to grasp the object)
            ...
            # Calculates the final forces to apply at the end effectors and
            # maps them to the finger motor torques
            ...
            if exit_before_timeout and all(
                np.abs(
                    np.subtract(
                        self.block_com_position,
                        desired_position)) < 0.005):
                break
    ...
    def follow_predefined_trajectory(self):
        ...
        self.move([0, 0, 0.25], offset=0.04,
                  timeout=2, exit_before_timeout=False)
        if self.mode == "simulation":
            self.finger.set_block_state(initial_position, [0, 0, 0, 1])
        self.move(initial_position, offset=0.04, timeout=2,
                  exit_before_timeout=False)  # reaching the object
        self.grasp()
        self.move(self.block_com_position[:2] + [0.21])  # picking up
        # moving forward
        self.move(self.landing_position[:2] + [self.block_com_position[2]])
        self.move(self.landing_position)  # moving down
        self.release()
        # moving away from the object after placing it
        self.move(self.landing_position[:2] + [0.25], offset=0.02)
        # moving to the centre (just using it for a better ending, for now)
        self.move([0, 0, 0.25], offset=0.02)
    ...
    def next_state(self):
        ...
        # interpolating an intermediate state for every "1" millisecs ahead
        ...
        if self.t % steps == 0:
            ...
            self.cube_q_next = np.vstack(
                [self.intermediate_state, self.intermediate_velocity])

        return np.vstack(
            [self.intermediate_state, self.intermediate_velocity])  # 13x1
    ...
    def compute_block_wrench(self):
        ...
        self.rotation_matrix = pinocchio.XYZQUATToSe3(
            self.cube_q_last).rotation  # local to world

        # this is to account for the discrepancy between the actual control
        # frequency and applied control frequency
        ...
        # this already has mass multiplied
        self._kc = np.matrix(np.diag(np.full(3, 100)))
        # this already has mass multiplied 0.005
        self._dc = np.matrix(np.diag(np.full(3, .001)))

        # feedback gains for the angular torques
        # this already has mass multiplied
        self._kb = np.matrix(np.diag(np.full(3, .01)))
        # this already has mass multiplied
        self._db = np.matrix(np.diag(np.full(3, .0000)))
    ...
    def compute_force_qp(self):
        ...
        for i in range(3):
            if self.cnt_array[i] == 0:
                continue
            F[3 * i: 3 * (i + 1)
              ] = self.rotation_matrix.dot(solx[3 * j: 3 * (j + 1)])
            j += 1
    ...
    def finger_impedance_control(self, unoptimised_forces=False):
        ...
        # Finger 0
        self.end_eff_des_vel[0] += - \
            np.linalg.norm(self.x_obs[0]) * object_state[12]
        self.end_eff_des_vel[1] += 0
        self.end_eff_des_vel[2] += np.linalg.norm(
            self.x_obs[0]) * object_state[10]

        # Finger 1
        self.end_eff_des_vel[3] += 0
        self.end_eff_des_vel[4] += np.linalg.norm(
            self.x_obs[1]) * object_state[12]
        self.end_eff_des_vel[5] += - \
            np.linalg.norm(self.x_obs[1]) * object_state[11]

        # Finger 2
        self.end_eff_des_vel[6] += 0
        self.end_eff_des_vel[7] += - \
            np.linalg.norm(self.x_obs[2]) * object_state[12]
        self.end_eff_des_vel[8] += np.linalg.norm(
            self.x_obs[2]) * object_state[11]

        for i in range(3):
            self.end_eff_des_vel[3 * i: 3 * (i + 1)] = self.rotation_matrix.dot(
                self.end_eff_des_vel[3 * i: 3 * (i + 1)])

        self.end_eff_obs_pos = [
            self.robot_data.oMf[tip_id].translation for tip_id in self.finger_tip_ids]
        ...
        force = self.Kp * (self.end_eff_des_pos - self.end_eff_obs_pos) + \
            self.Kd * (self.end_eff_des_vel - self.end_eff_obs_vel)
    ...
    def optimised_trajectory(self):
        ...
        self.move([0, 0, 0.03], offset=0.03, timeout=3,
                  exit_before_timeout=True)  # reaching the object
        self.grasp()
        ...
        for t in range(N):
            ...
            desired_contact_velocities[t, :] = np.resize(
                self.end_eff_des_vel, 9)
            observed_contact_velocities[t, :] = np.resize(
                self.end_eff_obs_vel, 9)
