...
def zero_torque_ctrl(robot, duration, logger=None, print_position=False):
    ...
    while step < duration:
        step += 1
        t = append_desired_action_(robot, desired_torque)
        if print_position:
...
def append_desired_action_(robot, desired_torque):
    action = two_joint.Action(torque=desired_torque)
    return robot.append_desired_action(action)
...
def hard_direction_change(robot, num_repetitions, torque, logger):
    ...
    t = append_desired_action_(robot, np.zeros(N_JOINTS))
    logger.set_time(t)
...
def main():
    ...
    finger_backend = blmc_robots.create_two_joint_backend(robot_data,
                                                          config_file_path)
