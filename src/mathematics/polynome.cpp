...
{
template <>
void TimePolynome<5>::set_parameters(double final_time,
                                     double init_pose,
                                     double init_speed,
                                     double final_pose)
{
    ...
    tmp = ft * ft * ft;
    coefficients_[3] =
        (-3.0 / 2.0 * ia * ft * ft - 6.0 * is * ft - 10.0 * ip + 10.0 * fp) /
        tmp;
    tmp = tmp * ft;
    coefficients_[4] =
        (3.0 / 2.0 * ia * ft * ft + 8.0 * is * ft + 15.0 * ip - 15.0 * fp) /
        tmp;
    tmp = tmp * ft;
    coefficients_[5] =
        (-1.0 / 2.0 * ia * ft * ft - 3.0 * is * ft - 6.0 * ip + 6.0 * fp) / tmp;
}
