...
void Solo8TI::initialize()
{
    ...
    for (unsigned i = 0; i < can_buses_.size(); ++i)
    {
        ...
        sliders_[i] = std::make_shared<blmc_drivers::AnalogSensor>(
            can_motor_boards_[i], 1);
    }
...
void Solo8TI::acquire_sensors()
{
    ...
    motor_board_enabled_[0] =
        static_cast<bool>(FL_status.system_enabled);  // FL board
    motor_board_enabled_[1] =
        static_cast<bool>(FR_status.system_enabled);  // FR board
    motor_board_enabled_[2] =
        static_cast<bool>(HL_status.system_enabled);  // HL board
    motor_board_enabled_[3] =
        static_cast<bool>(HR_status.system_enabled);  // HR board

    motor_board_errors_[0] =
        static_cast<int>(FL_status.error_code);  // FL board
    motor_board_errors_[1] =
        static_cast<int>(FR_status.error_code);  // FR board
    motor_board_errors_[2] =
        static_cast<int>(HL_status.error_code);  // HL board
    motor_board_errors_[3] =
        static_cast<int>(HR_status.error_code);  // HR board
...
void Solo8TI::send_target_joint_torque(
...
bool Solo8TI::calibrate(const Vector8d& home_offset_rad)
{
