...
void TestBench8Motors::initialize()
{
    ...
    // table
    board0_motor0_ =
        std::make_shared<blmc_drivers::SafeMotor>(board0_, 0, max_current_);
    board0_motor1_ =
        std::make_shared<blmc_drivers::SafeMotor>(board0_, 1, max_current_);
    ...
    board1_motor0_ =
        std::make_shared<blmc_drivers::SafeMotor>(board1_, 0, max_current_);
    board1_motor1_ =
        std::make_shared<blmc_drivers::SafeMotor>(board1_, 1, max_current_);
    ...
    board2_motor0_ =
        std::make_shared<blmc_drivers::SafeMotor>(board2_, 0, max_current_);
    board2_motor1_ =
        std::make_shared<blmc_drivers::SafeMotor>(board2_, 1, max_current_);
    ...
    board3_motor0_ =
        std::make_shared<blmc_drivers::SafeMotor>(board3_, 0, max_current_);
    board3_motor1_ =
        std::make_shared<blmc_drivers::SafeMotor>(board3_, 1, max_current_);
...
void TestBench8Motors::acquire_sensors()
{
    ...
    motor_positions_(0) =
        board0_motor0_->get_measurement(mi::position)->newest_element();
    motor_positions_(1) =
        board0_motor1_->get_measurement(mi::position)->newest_element();
    motor_positions_(2) =
        board1_motor0_->get_measurement(mi::position)->newest_element();
    motor_positions_(3) =
        board1_motor1_->get_measurement(mi::position)->newest_element();
    motor_positions_(4) =
        board2_motor0_->get_measurement(mi::position)->newest_element();
    motor_positions_(5) =
        board2_motor1_->get_measurement(mi::position)->newest_element();
    motor_positions_(6) =
        board3_motor0_->get_measurement(mi::position)->newest_element();
    motor_positions_(7) =
        board3_motor1_->get_measurement(mi::position)->newest_element();

    // acquire the motors velocities
    motor_velocities_(0) =
        board0_motor0_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(1) =
        board0_motor1_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(2) =
        board1_motor0_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(3) =
        board1_motor1_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(4) =
        board2_motor0_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(5) =
        board2_motor1_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(6) =
        board3_motor0_->get_measurement(mi::velocity)->newest_element();
    motor_velocities_(7) =
        board3_motor1_->get_measurement(mi::velocity)->newest_element();

    // acquire the motors current
    motor_currents_(0) =
        board0_motor0_->get_measurement(mi::current)->newest_element();
    motor_currents_(1) =
        board0_motor1_->get_measurement(mi::current)->newest_element();
    motor_currents_(2) =
        board1_motor0_->get_measurement(mi::current)->newest_element();
    motor_currents_(3) =
        board1_motor1_->get_measurement(mi::current)->newest_element();
    motor_currents_(4) =
        board2_motor0_->get_measurement(mi::current)->newest_element();
    motor_currents_(5) =
        board2_motor1_->get_measurement(mi::current)->newest_element();
    motor_currents_(6) =
        board3_motor0_->get_measurement(mi::current)->newest_element();
    motor_currents_(7) =
        board3_motor1_->get_measurement(mi::current)->newest_element();

    // acquire the motors encoder index
    motor_encoder_indexes_(0) =
        board0_motor0_->get_measurement(mi::encoder_index)->length() > 0
            ? board0_motor0_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(1) =
        board0_motor1_->get_measurement(mi::encoder_index)->length() > 0
            ? board0_motor1_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(2) =
        board1_motor0_->get_measurement(mi::encoder_index)->length() > 0
            ? board1_motor0_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(3) =
        board1_motor1_->get_measurement(mi::encoder_index)->length() > 0
            ? board1_motor1_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(4) =
        board2_motor0_->get_measurement(mi::encoder_index)->length() > 0
            ? board2_motor0_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(5) =
        board2_motor1_->get_measurement(mi::encoder_index)->length() > 0
            ? board2_motor1_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(6) =
        board3_motor0_->get_measurement(mi::encoder_index)->length() > 0
            ? board3_motor0_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");

    motor_encoder_indexes_(7) =
        board3_motor1_->get_measurement(mi::encoder_index)->length() > 0
            ? board3_motor1_->get_measurement(mi::encoder_index)
                  ->newest_element()
            : std::nan("");
...
void TestBench8Motors::send_target_current(const Vector8d target_currents)
{
